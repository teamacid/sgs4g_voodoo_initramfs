...
detect_supported_model_and_setup_partitions()
{
	...
	if test "$model" != ""; then
		...
		if test "$model" = 'tmo-galaxys4g'; then
			dbdata_partition=''
			data_on_emmc=0
...
verify_voodoo_install()
{
	for x in /sbin/fat.format /system/bin/fat.format; do
		...
		if ! test -L "$prefix/fat.format" && test -x "$prefix/fat.format_wrapper.sh" ; then
			...
			chmod 755 "$prefix/fat.format.real" "$prefix/fat.format_wrapper.sh"
			#ln -s "$prefix/fat.format_wrapper.sh" "$prefix/fat.format"
			cp -f "$prefix/fat.format_wrapper.sh" "$prefix/fat.format"
			log "fat.format renamed to fat.format.real & fat.format_wrapper.sh copied to fat.format"
		fi
	done
}
...
ext4_format()
{
	common_mkfs_ext4_options='^resize_inode,^ext_attr,^huge_file,^has_journal'
	case $resource in
		# tune system inode number to fit available space in RFS and Ext4
		system)	mkfs_options="-O $common_mkfs_ext4_options -N 7500"  ;;
		cache)	mkfs_options="-O $common_mkfs_ext4_options -N 800"  ;;
		data)	mkfs_options="-O $common_mkfs_ext4_options" ;;
		dbdata)	mkfs_options="-O $common_mkfs_ext4_options" ;;
	esac
	
	mkfs.ext4 -F $mkfs_options -T default $partition
	# force check the filesystem after 100 mounts or 100 days
	tune2fs -c 1 -m 0 -o journal_data_writeback -L $resource $partition
}
...
readd_boot_animation()
{
	...
	if test -f /data/local/bootanimation.zip || test -f /system/media/bootanimation.zip; then
		test -f /system/bin/bootanimation && chmod 755 /system/bin/bootanimation
...
letsgo()
{
	...
	mount_ data
	readd_boot_animation
	# HACK: for tvoutserver
	# tvoutserver must not run in recovery
        if ! in_recovery; then
		echo >> init.herring.rc
		echo 'service tvout /system/bin/tvoutserver
    		      user system
		      group graphics' >> init.herring.rc
	fi
	umount /data
